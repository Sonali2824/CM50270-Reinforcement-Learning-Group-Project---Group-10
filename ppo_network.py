...
class Actor(torch.nn.Module):
  ...
  def __init__(self, hyperparameters):
    ...
    self.convolution_2d_1 = torch.nn.Conv2d(hyperparameters["no_frames_to_network"], 16, 8, 4)
    self.relu_1 = torch.nn.ReLU()
    self.convolution_2d_2 = torch.nn.Conv2d(16, 32, 4, 2)
    self.relu_2 = torch.nn.ReLU()
    self.fully_connected_3 = torch.nn.Linear(2592, 256)
  ...
  def forward(self, state):
    ...
    x_input = state
    state = self.relu_1(self.convolution_2d_1(state))
    state = self.relu_2(self.convolution_2d_2(state))
    x_1 = state
    state = state.view(state.size()[0], -1)
    state = self.relu_3(self.fully_connected_3(state))
